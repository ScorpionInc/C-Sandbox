...
void fprint_signal_debug(FILE* const p_file)
{
	...
	for (int iii = 0; iii < SI_SIGNALS_MAX; iii++)
	{
		...
		{
			fprintf(p_file, "[%24s][%2d] Default signal handler\n",
				p_sig_desc, iii
			);
		}
		else if (SIG_IGN == query.sa_handler)
		{
			fprintf(p_file, "[%24s][%2d] Ignored signal\n", p_sig_desc, iii);
		}
		else
		{
			fprintf(p_file, "[%24s][%2d] Custom handler: %p\n",
				p_sig_desc, iii, (void*)query.sa_handler
			);
		}
	}
END:
